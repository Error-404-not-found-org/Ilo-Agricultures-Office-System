...
const Technicians = () => {
    ...
    const getStatusColor = (status) => {
        switch (status?.toLowerCase()) {
            ...
            case 'on-site': return 'badge-warning text-white';
            case 'on-leave': return 'badge-error text-white';
            ...
            {technicians?.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {technicians?.map((tech) => (
                        <div key={tech._id} className="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow duration-300 rounded-xl">
                            <figure className="px-6 pt-6">
                                <div className="avatar">
                                    <div className="w-24 rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
                                        <img 
                                            src={tech.imageUrl || "https://ui-avatars.com/api/?name=" + tech.name} 
                                            alt={tech.name} 
                                        />
                                    </div>
                                </div>
                            ...
                            <div className="card-body items-center text-center">
                                <h2 className="card-title text-xl font-bold">{tech.name}</h2>
                                <div className={`badge ${getStatusColor(tech.status)} badge-lg mb-2 capitalize`}>
                                    {tech.status || 'Unknown'}
                                </div>
                                ...
                                <div className="space-y-1 w-full text-sm text-gray-600 mb-4">
                                    ...
                                    <div className="flex justify-between pb-1">
                                        ...
                                        <span className="truncate max-w-[150px]">{tech.address?.city || 'N/A'}, {tech.address?.province || ''}</span>
                                    </div>
                                </div>

                                <div className="card-actions w-full justify-center gap-2">
                                    <button className="btn btn-sm btn-outline btn-info">View Profile</button>
