...
test("Breeding corrections: pregnancy and calving correction routes are admin-only", () => {
  assert.match(
    routes,
    /router\.patch\([\s\S]*?"\/pregnancy-checks\/:id\/correct"[\s\S]*?requireRole\(\["admin"\]\)[\s\S]*?correctPregnancyCheck/,
  );
  assert.match(
    routes,
    /router\.patch\([\s\S]*?"\/calvings\/:id\/correct"[\s\S]*?requireRole\(\["admin"\]\)[\s\S]*?correctCalving/,
  );
});

test("Breeding corrections: legacy delete endpoints cannot erase official records", () => {
  assert.match(routes, /delete\(\s*\"\/pregnancy-checks\/:id\",[\s\S]*?requireRole\(\[\"admin\"\]\)/);
  assert.match(routes, /delete\(\s*\"\/calvings\/:id\",[\s\S]*?requireRole\(\[\"admin\"\]\)/);
