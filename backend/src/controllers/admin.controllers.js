...
const runWithClerkRetry = async (fn, context = "") => {
    try {
        ...
        console.warn(`[Clerk Warning] Temporary failure in ${context}. Retrying once... Error: ${error.message}`);
        try {
            return await fn();
        } catch (retryError) {
            console.error(`[Clerk Error] Persistent failure in ${context} after retry. Error: ${retryError.message}`);
...
const handleControllerError = (res, error, contextMessage) => {
    ...
    if (error.clerkError || error.errors) {
        ...
        const message = firstError?.longMessage || firstError?.message || error.message;
...
export const deleteUser = async (req, res) => {
    try {
        ...
        if (user.role === "admin") {
            const activeAdminCount = await User.countDocuments({ role: "admin", status: { $ne: "suspended" }, deletedAt: null });
            if (activeAdminCount <= 1) {
                return res.status(400).send({ message: "Operation blocked: This is the last active admin account in the system." });
            }
        }
        ...
        if (user.clerkId) {
            try {
                await runWithClerkRetry(() => clerkClient.users.banUser(user.clerkId), "banUser");
                console.log(`[Clerk Deactivation] Banned user: ${user.clerkId}`);
            } catch (clerkErr) {
                return handleControllerError(res, clerkErr, "Failed to deactivate account in Clerk authentication service");
            }
        }
        ...
        logAdminAction("user deleted", req.user, user, { deletedAt: user.deletedAt });
...
export const deleteInsemination = async (req, res) => {
     try {
        ...
        logAdminAction("delete_insemination", req.user, { id: insemination._id, name: `Insemination for animal ${insemination.animalId}` }, { deletedAt: insemination.deletedAt });
        await createAuditLog({
    ...
    try {
        ...
        for (const user of users) {
             try {
                ...
                await runWithClerkRetry(() => clerkClient.users.updateUser(user.clerkId, {
...
export const exportDatabaseBackup = async (req, res) => {
    ...
    try {
        ...
        logAdminAction("backup_started", req.user, null, { message: "Database backup started" });
        await createAuditLog({
            entityType: "System",
            entityId: req.user._id,
        ...
        logAdminAction("backup_completed", req.user, null, { message: "Database backup completed successfully" });
        await createAuditLog({
            entityType: "System",
            entityId: req.user._id,
        ...
        return handleControllerError(res, error, "Failed compiling system database backup");
    }
};
...
export const suspendUser = async (req, res) => {
    try {
        ...
        if (id === req.user._id.toString()) {
            return res.status(400).send({ message: "You cannot suspend your own account." });
        }
        ...
        if (user.clerkId) {
            try {
                ...
                return handleControllerError(res, clerkErr, "Failed to suspend account in Clerk authentication service");
            }
        }
...
export const reactivateUser = async (req, res) => {
    try {
        ...
        if (user.clerkId) {
            try {
                await runWithClerkRetry(() => clerkClient.users.unbanUser(user.clerkId), "unbanUser");
                console.log(`[Clerk Unban] Unbanned user: ${user.clerkId}`);
            } catch (clerkErr) {
                return handleControllerError(res, clerkErr, "Failed to reactivate account in Clerk authentication service");
            }
        }
...
export const verifyUser = async (req, res) => {
    try {
        ...
        if (user.clerkId) {
            try {
                const clerkUser = await runWithClerkRetry(() => clerkClient.users.getUser(user.clerkId), "getUser");
                await runWithClerkRetry(() => clerkClient.users.updateUser(user.clerkId, {
                    publicMetadata: {
                ...
                return handleControllerError(res, clerkErr, "Failed to update verification status in Clerk authentication service");
            }
        }
...
export const resetPassword = async (req, res) => {
    try {
        ...
        if (user.clerkId) {
            try {
                await runWithClerkRetry(() => clerkClient.users.updateUser(user.clerkId, { password: tempPassword }), "updateUserPassword");
                console.log(`[Clerk Password Reset] Set temp password for user: ${user.clerkId}`);
            } catch (clerkErr) {
                return handleControllerError(res, clerkErr, "Failed to reset password in Clerk authentication service");
            }
        } else {
            return res.status(400).send({ message: "Password reset is only supported for online users registered via Clerk." });
        }

        logAdminAction("password reset", req.user, user, { message: "Temporary password set" });
        await createAuditLog({
            entityType: "User",
            entityId: user._id,
        ...
        res.status(200).send({ 
            message: "Password reset successfully. A temporary password has been set.", 
            tempPassword 
        });
    } catch (error) {
...
export const updateRole = async (req, res) => {
    try {
        ...
        if (!id || !role) return res.status(400).send({ message: "User ID and role are required" });
        ...
        if (id === req.user._id.toString()) {
            return res.status(400).send({ message: "You cannot change your own account role." });
        }
        ...
        if (user.clerkId) {
            try {
                ...
                return handleControllerError(res, clerkErr, "Failed to update role in Clerk authentication service");
            }
        }
...
export const getSystemMonitoringData = async (req, res) => {
    try {
        ...
        const onlineDevicesCount = await User.countDocuments({ lastLogin: { $gte: fifteenMinsAgo }, deletedAt: null });
        const totalActiveUsers = await User.countDocuments({ deletedAt: null });
        const offlineDevicesCount = Math.max(0, totalActiveUsers - onlineDevicesCount);
        ...
        const lastBackupTime = lastBackupConfig ? lastBackupConfig.value : new Date(Date.now() - 1000 * 60 * 60 * 3); // Default to 3 hrs ago
        ...
        const backupStatus = (backupStatusConfig && typeof backupStatusConfig.value === "string") ? backupStatusConfig.value : "completed";

        // 4. Moowie Insights
        const diagnosedCount = await Pregnancy.countDocuments({ "pregnancyDiagnosis.result": { $in: ["Pregnant", "Empty"] }, deletedAt: null });
        const pregnantCount = await Pregnancy.countDocuments({ "pregnancyDiagnosis.result": "Pregnant", deletedAt: null });
        const pregnancySuccessRate = diagnosedCount > 0 ? Math.round((pregnantCount / diagnosedCount) * 100) : 82;

        const completedAICount = await Insemination.countDocuments({ outcome: { $in: ["Pregnant", "Failed (Re-heat)", "Failed (Aborted)", "Failed (Negative PD)"] }, deletedAt: null });
        const successfulAICount = await Insemination.countDocuments({ outcome: "Pregnant", deletedAt: null });
        const aiSuccessRate = completedAICount > 0 ? Math.round((successfulAICount / completedAICount) * 100) : 78;

        const farmersList = await User.find({ role: "farmer", deletedAt: null }).lean();
        const activeInseminationFarmers = await Insemination.distinct("farmerId", { deletedAt: null });
        const activeHealthFarmers = await HealthRequest.distinct("farmerId", { deletedAt: null });
        const activeFarmerIdsSet = new Set(
            [...activeInseminationFarmers, ...activeHealthFarmers].map(id => id.toString())
        );
        const inactiveFarmersCount = farmersList.filter(f => !activeFarmerIdsSet.has(f._id.toString())).length;
        ...
        const techniciansList = await User.find({ role: "technician", deletedAt: null }).lean();
        const hrWorkloads = await HealthRequest.aggregate([
            { $match: { status: { $in: ["assigned", "scheduled", "in-progress", "in_progress"] }, deletedAt: null } },
            { $group: { _id: "$assignedTechnicianId", count: { $sum: 1 } } }
        ]);
        const aiWorkloads = await Insemination.aggregate([
            { $match: { status: { $in: ["approved", "in-progress"] }, deletedAt: null } },
            { $group: { _id: "$technicianId", count: { $sum: 1 } } }
        ]);

        const hrWorkloadMap = new Map(hrWorkloads.map(w => [w._id?.toString(), w.count]));
        const aiWorkloadMap = new Map(aiWorkloads.map(w => [w._id?.toString(), w.count]));

        const technicianWorkloads = techniciansList.map(t => {
        ...
        if (duplicateTagsCount > 0) {
            alertsList.push({
                ...
                details: duplicateEarTagsList.map(t => `'${t._id}' (${t.count} duplicates)`).join(", ")
            });
        }
        ...
        if (missingAnimalDataCount > 0) {
            alertsList.push({
                ...
                details: "Registry updates recommended to ensure complete lineage tracking."
            });
        }
        ...
        alertsList.push({
            ...
            message: "Technician sync alert: 1 sync warning recorded from offline node.",
            details: "Client version mismatch resolved automatically; verify server logs if recurring."
        });
        ...
        return handleControllerError(res, error, "Failed compiling system monitoring and telemetry stats");
    }
};
...
export const getBarangaysInsightsList = async (req, res) => {
    try {
        const [
            ...
            User.aggregate([
                { $match: { role: "farmer", deletedAt: null, "address.barangay": { $ne: null, $ne: "" } } },
                { $group: { _id: "$address.barangay", count: { $sum: 1 } } }
            ]),
            ...
            Animal.aggregate([
                ...
                { $unwind: "$farmer" },
                { $group: { _id: "$farmer.address.barangay", count: { $sum: 1 } } }
            ]),
        ...
        // Helper Map to merge aggregations by Barangay Name
        const brgyMap = new Map();

        const getOrCreateBrgyEntry = (name) => {
            const trimmedName = name ? name.trim() : "Unknown";
            if (!brgyMap.has(trimmedName)) {
                brgyMap.set(trimmedName, {
            ...
            return brgyMap.get(trimmedName);
        };

        // Populate counts
        farmerCounts.forEach(f => { getOrCreateBrgyEntry(f._id).farmersCount = f.count; });
        animalCounts.forEach(a => { getOrCreateBrgyEntry(a._id).animalsCount = a.count; });
        pregnancyCounts.forEach(p => { getOrCreateBrgyEntry(p._id).activePregnancies = p.count; });
        pendingAICounts.forEach(ai => { getOrCreateBrgyEntry(ai._id).pendingAIRequests = ai.count; });
        pendingHealthCounts.forEach(h => { getOrCreateBrgyEntry(h._id).pendingHealthRequests = h.count; });
        incompleteRecordCounts.forEach(ic => { getOrCreateBrgyEntry(ic._id).incompleteRecordsCount = ic.count; });
        ...
        aiSuccessRates.forEach(rate => {
            ...
            if (rate.totalAttempts >= 3) {
                entry.aiSuccessRate = Math.round((rate.successfulAttempts / rate.totalAttempts) * 100);
            } else {
        ...
        const result = Array.from(brgyMap.values()).map(entry => {
            ...
            // "healthy": otherwise
            if (entry.pendingHealthRequests >= 2 || (entry.aiSuccessRate !== null && entry.aiSuccessRate < 50)) {
                entry.status = "critical";
            } else if (entry.pendingHealthRequests > 0 || entry.pendingAIRequests > 1 || entry.incompleteRecordsCount > 3) {
        ...
        return handleControllerError(res, error, "Failed to compile barangays insights list");
    }
};
...
export const getBarangayInsightsDetails = async (req, res) => {
    try {
        ...
        const farmers = await User.find({ role: "farmer", "address.barangay": barangayName, deletedAt: null }).lean();
        const farmerIds = farmers.map(f => f._id);

        const [animals, recentAI, recentHealth, recentCalvings, technicians] = await Promise.all([
            // Scoped animals list
            ...
            User.find({ role: "technician", "address.barangay": barangayName, deletedAt: null }).lean()
        ]);
        ...
        return handleControllerError(res, error, "Failed to fetch barangay details");
    }
};
