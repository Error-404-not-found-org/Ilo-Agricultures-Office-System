...
export function normalizeSpecies(species?: string): SpeciesType {
  ...
  const aliases: Record<string, SpeciesType> = {
    beef: "Beef Cattle",
...
export function verifyPostpartumWindow(
  lastCalvingDate: Date | string,
  targetActionDate: Date | string,
  species: string,
  breed?: string,
): {
  isSafe: boolean;
