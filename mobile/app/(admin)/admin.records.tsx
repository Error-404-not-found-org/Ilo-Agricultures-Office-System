...
export default function AdminRecordsScreen() {
  ...
  return (
    <View className="flex-1 bg-[#F0F4FF]">
      ...
      <View className="flex-1 bg-[#F0F4FF] rounded-t-[32px] px-6 pt-6 mt-2 shadow-lg" style={{ elevation: 8 }}>
        <Text className="text-[22px] font-bold text-slate-800 mb-4">Records</Text>
        ...
        <View className="flex-row gap-x-2 mb-5">
          {TABS.map((tab, i) => (
            <TouchableOpacity
              ...
              className={`flex-1 py-2.5 rounded-xl items-center border ${activeTab === i ? 'border-[#1e3a5f]' : 'bg-white border-slate-200'}`}
              style={activeTab === i ? { backgroundColor: PRIMARY } : {}}
            >
              <Text
                className={`text-xs font-bold ${activeTab === i ? 'text-white' : 'text-slate-500'}`}
              >
                {tab}
              </Text>
            </TouchableOpacity>
        ...
        {isLoading ? (
          ...
          >
            {currentData.map((item: any) => (
              <View key={item._id} className="bg-white rounded-[20px] p-4 mb-3 border border-slate-100 shadow-sm">
                  ...
                  <View className="ml-12 gap-y-1">
                    {item.inseminationDate && (
                      <Text className="text-[12px] text-slate-600">
                        Date: <Text className="font-semibold">{new Date(item.inseminationDate).toLocaleDateString()}</Text>
                      </Text>
                    )}
                    {item.sireCode && (
                      <Text className="text-[12px] text-slate-600">
                        Sire: <Text className="font-semibold">{item.sireCode}</Text>
                      </Text>
                    )}
                    {item.status && (
                      <View className="self-start px-2 py-0.5 rounded-full bg-blue-50 mt-1">
                        <Text className="text-blue-700 text-[10px] font-bold uppercase">{item.status}</Text>
                      </View>
                    )}
                  </View>
                )}
                {activeTab === 1 && (
                  <View className="ml-12 gap-y-1">
                    {item.pregnancyDiagnosis?.date && (
                      <Text className="text-[12px] text-slate-600">
                        Check Date: <Text className="font-semibold">{new Date(item.pregnancyDiagnosis.date).toLocaleDateString()}</Text>
                      </Text>
                    )}
                    {item.pregnancyDiagnosis?.result && (
                      <View className={`self-start px-2 py-0.5 rounded-full mt-1 ${item.pregnancyDiagnosis.result === 'Pregnant' ? 'bg-emerald-50' : 'bg-slate-100'}`}>
                        <Text className={`text-[10px] font-bold uppercase ${item.pregnancyDiagnosis.result === 'Pregnant' ? 'text-emerald-700' : 'text-slate-500'}`}>
                          {item.pregnancyDiagnosis.result}
                        </Text>
                      </View>
                    )}
                ...
                {activeTab === 2 && (
                  <View className="ml-12 gap-y-1">
                    {item.date && (
                      <Text className="text-[12px] text-slate-600">
                        Calving Date: <Text className="font-semibold">{new Date(item.date).toLocaleDateString()}</Text>
                      </Text>
                    )}
                    {item.calfSex && (
                      <Text className="text-[12px] text-slate-600">
                        Calf Sex: <Text className="font-semibold">{item.calfSex}</Text>
                      </Text>
                    )}
                  </View>
                )}
