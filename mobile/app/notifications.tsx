    ...
    if (type === "ai-request")
      return (
        <View
          ...
          className="w-12 h-12 dark:bg-emerald-900/30 rounded-full items-center justify-center"
        >
      ...
      <View className="flex-1 ml-4 justify-center">
        <View className="flex-row justify-between items-start mb-1">
          <Text
            ...
            className={`text-base flex-1 mr-2 ${item.isRead ? "text-slate-700 dark:text-slate-300" : "text-slate-900 dark:text-white"}`}
          >
            {item.title}
          </Text>
          {!item.isRead && (
        ...
        <Text
          ...
          className={`text-[13px] leading-5 ${item.isRead ? "text-slate-500 dark:text-slate-400" : "text-slate-600 dark:text-slate-300 font-medium"}`}
        >
          {item.message}
        </Text>
        <Text
          style={{ fontFamily: "Outfit_700Bold" }}
          className="text-slate-400 dark:text-slate-500 text-xs mt-2 font-medium"
        >
    ...
    <View className="flex-1 bg-[#F9FAFB] dark:bg-slate-950">
      ...
      >
        <View className="flex-row justify-between items-center mb-6">
          <Text
            style={{ fontFamily: "Outfit_900Black" }}
            className="text-[20px] text-slate-800 dark:text-white"
          >
            Recent Updates
          </Text>
          ...
          <View className="flex-row items-center gap-3">
            <TouchableOpacity onPress={markAllAsRead}>
              <Text
                style={{ fontFamily: "Outfit_800ExtraBold" }}
                className={`text-xs ${THEME.markRead}`}
              >
                Mark read
              </Text>
            </TouchableOpacity>

            <Text className="text-slate-300 dark:text-slate-700 text-xs">
              •
            </Text>
