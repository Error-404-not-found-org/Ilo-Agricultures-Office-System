...
export default function RegisterClientScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // We call the same endpoint the web dashboard uses
      await api.post('/user/sync-manual', payload);
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView className="flex-1 px-6 pt-6" contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
          ...
          <View className="bg-emerald-50 rounded-2xl p-4 mb-6 border border-emerald-100 flex-row items-center">
             ...
             <Text style={{ fontFamily: 'Outfit_600SemiBold' }} className="text-emerald-800 text-xs flex-1">
               Register a new farmer for immediate service. An offline profile will be created.
             </Text>
          </View>

          {/* PERSONAL DETAILS */}
          <Text className="font-outfit-bold text-slate-400 uppercase text-[10px] tracking-widest mb-3 ml-1">Personal Details</Text>
          <View className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm mb-6 gap-4">
             <View>
                <View className="flex-row justify-between mb-1 ml-1">
                   <Text className="text-slate-700 text-[11px] font-outfit-bold uppercase">First Name *</Text>
                   {errors.firstName && <Text className="text-red-500 text-[9px] font-outfit-bold">{errors.firstName}</Text>}
                </View>
                <TextInput
                  className={`bg-slate-50 border ${errors.firstName ? 'border-red-300' : 'border-slate-100'} rounded-xl p-3.5 text-slate-800 font-outfit-medium`}
                  placeholder="Juan"
             ...
             <View>
                <View className="flex-row justify-between mb-1 ml-1">
                   <Text className="text-slate-700 text-[11px] font-outfit-bold uppercase">Last Name *</Text>
                   {errors.lastName && <Text className="text-red-500 text-[9px] font-outfit-bold">{errors.lastName}</Text>}
                </View>
                <TextInput
                  className={`bg-slate-50 border ${errors.lastName ? 'border-red-300' : 'border-slate-100'} rounded-xl p-3.5 text-slate-800 font-outfit-medium`}
          ...
          {/* CONTACT & ADDRESS */}
          <Text className="font-outfit-bold text-slate-400 uppercase text-[10px] tracking-widest mb-3 ml-1">Contact & Address</Text>
          <View className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm mb-6 gap-4">
             <View>
                <View className="flex-row justify-between mb-1 ml-1">
                   <Text className="text-slate-700 text-[11px] font-outfit-bold uppercase">Phone Number *</Text>
                   {errors.phoneNumber && <Text className="text-red-500 text-[9px] font-outfit-bold">{errors.phoneNumber}</Text>}
                </View>
                <TextInput
                  className={`bg-slate-50 border ${errors.phoneNumber ? 'border-red-300' : 'border-slate-100'} rounded-xl p-3.5 text-slate-800 font-outfit-medium`}
             ...
             <View>
                <View className="flex-row justify-between mb-1 ml-1">
                   <Text className="text-slate-700 text-[11px] font-outfit-bold uppercase">Email (Optional)</Text>
                   {errors.email && <Text className="text-red-500 text-[9px] font-outfit-bold">{errors.email}</Text>}
                </View>
                <TextInput
                  className={`bg-slate-50 border ${errors.email ? 'border-red-300' : 'border-slate-100'} rounded-xl p-3.5 text-slate-800 font-outfit-medium`}
             ...
             <View className="flex-row gap-3">
                <View className="flex-1">
                   <View className="flex-row justify-between mb-1 ml-1">
                     <Text className="text-slate-700 text-[11px] font-outfit-bold uppercase">Barangay *</Text>
                     {errors.barangay && <Text className="text-red-500 text-[9px] font-outfit-bold">{errors.barangay}</Text>}
                   </View>
                   <TouchableOpacity
                     onPress={() => setShowBrgyModal(true)}
                     className={`bg-slate-50 border ${errors.barangay ? 'border-red-300' : 'border-slate-100'} rounded-xl p-3.5 flex-row justify-between items-center`}
                   >
                     <Text className={`font-outfit-medium ${formData.barangay ? 'text-slate-800' : 'text-slate-400'}`}>
                ...
                <View className="flex-1">
                   <Text className="text-slate-700 text-[11px] font-outfit-bold mb-1.5 ml-1 uppercase">Municipality</Text>
                   <TextInput
                     className="bg-slate-100 border border-slate-100 rounded-xl p-3.5 text-slate-400 font-outfit-medium"
                     value="OTON"
                     editable={false}
                   />
                </View>
