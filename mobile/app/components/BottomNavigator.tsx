          ...
          <TouchableWithoutFeedback>
            <View className="bg-white dark:bg-slate-900 rounded-t-[40px] px-6 pt-8 pb-12" style={[styles.modalShadow, { paddingBottom: insets.bottom + 20 }]}>
              ...
              <View className="flex-row justify-between items-center mb-8">
                <Text className="text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Quick Actions</Text>
                <TouchableOpacity
                  onPress={() => setModalVisible(false)}
...
const TabItem = ({ icon: Icon, label, isFocused, onPress, colorScheme }: any) => (
  ...
  >
    <View
      style={
        isFocused
          ? { backgroundColor: colorScheme === 'dark' ? '#134e4a' : '#f0fdfa', width: 52, height: 32, borderRadius: 10, alignItems: 'center', justifyContent: 'center' }
...
const ModalAction = ({ icon, label, onPress }: { icon: any; label: string; onPress?: () => void }) => (
  ...
  >
    <View className="w-14 h-14 bg-teal-50 dark:bg-teal-900/30 rounded-full items-center justify-center mb-3">{icon}</View>
