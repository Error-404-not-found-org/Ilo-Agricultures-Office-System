...
export function AnimalSummaryCard({ animal, onPress, nextAction, alert, variant = "list", cardWidth = 150 }: { animal: any; onPress?: () => void; nextAction?: string; alert?: string; variant?: "list" | "preview"; cardWidth?: number }) {
  ...
  const secondaryMetadata = [animal.breed, animal.species]
    .filter(Boolean)
  ...
  if (variant === "preview") {
    return (
      <TouchableOpacity
        ...
        style={{ width: cardWidth, minHeight: 214, borderRadius: 8, backgroundColor: colors.card, borderColor: colors.border }}
      >
        ...
        <View className="p-3" style={{ minHeight: 116 }}>
          ...
          <Text numberOfLines={1} className="mt-1" style={{ color: colors.textSecondary, fontFamily: "Outfit_500Medium", fontSize: 10 }}>{secondaryMetadata}</Text>
