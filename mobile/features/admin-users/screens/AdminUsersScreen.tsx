...
import { SearchBar, AsyncState, StatusBadge, Pagination, CustomDialog } from '@/components/shared';
...
export default function AdminUsersScreen() {
  ...
  const [activeUserForDialog, setActiveUserForDialog] = React.useState<UserItem | null>(null);
  ...
  const headerElement = (
    <View style={{ marginBottom: 8 }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Text style={{ fontSize: 24, fontFamily: 'Outfit_800ExtraBold', color: colors.textPrimary }}>
          User Directory
        </Text>
        <TouchableOpacity
        ...
        >
          ...
          <Text style={{ color: '#fff', fontFamily: 'Outfit_700Bold', fontSize: 12 }}>Create</Text>
        </TouchableOpacity>
      </View>
    ...
    <ScreenLayout edges={[]}>
      ...
      >
        <FlatList
          ...
          ListEmptyComponent={() => {
            ...
            if (isError) return <AsyncState state="error" message="Failed to load users." onAction={refetch} />;
            return <AsyncState state="empty" title="No users found" message="Try a different search or filter." />;
          }}
          renderItem={({ item }) => (
            <UserCard
              item={item}
      ...
      <CustomDialog
        ...
        icon={
          ...
          >
            <MaterialCommunityIcons name="account-cog" size={26} color="#2563eb" />
          </View>
        }
        actions={[
          {
            ...
            ? [
                {
                  ...
                  onPress: () => {
                    setDialogVisible(false);
                    if (activeUserForDialog) handleVerifyUser(activeUserForDialog);
                  }
                }
              ]
            : []),
          {
            text: activeUserForDialog?.status === "suspended" ? "Reactivate User" : "Suspend User",
            variant: activeUserForDialog?.status === "suspended" ? "secondary" : "danger",
            onPress: () => {
              setDialogVisible(false);
              if (activeUserForDialog) handleSuspendUser(activeUserForDialog);
            }
          },
          {
            text: "Cancel",
    ...
    >
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 }}>
        {hasImage ? (
          ...
          >
            <Text style={{ color: isDark ? roleStyle.accent : roleStyle.text, fontFamily: 'Outfit_800ExtraBold', fontSize: 18 }}>
              {item.name?.charAt(0)?.toUpperCase() || '?'}
            </Text>
          </View>
        )}
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 17, fontFamily: 'Outfit_800ExtraBold', color: colors.textPrimary }}>
            {item.name || 'No Name'}
          </Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
            <StatusBadge label={item.role} />
            {item.isVerified && <StatusBadge label="Verified" variant="success" />}
            {item.status === 'suspended' && <StatusBadge label="Suspended" variant="danger" />}
            {!item.isVerified && <StatusBadge label="Unverified" variant="warning" />}
          </View>
        </View>
      </View>
      ...
      >
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          ...
          <Text style={{ fontSize: 13, fontFamily: 'Outfit_600SemiBold', color: colors.textSecondary }}>
            {item.address?.barangay
              ? `${item.address.barangay}${item.address.city ? `, ${item.address.city}` : ''}`
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          ...
          <Text style={{ fontSize: 13, fontFamily: 'Outfit_500Medium', color: colors.textMuted }}>
            Last Active: {item.lastLogin ? new Date(item.lastLogin).toLocaleDateString() : 'Never'}
          </Text>
        </View>
        ...
        {item.role === 'farmer' && (
          ...
          >
            ...
            <Text style={{ fontSize: 13, fontFamily: 'Outfit_500Medium', color: colors.textSecondary }}>
              Animal Count:{' '}
              <Text style={{ fontFamily: 'Outfit_700Bold', color: colors.textPrimary }}>{animalCount}</Text>
            </Text>
          </View>
        )}
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              ...
              <Text style={{ fontSize: 13, fontFamily: 'Outfit_500Medium', color: colors.textSecondary }}>
                ...
                <Text style={{ fontFamily: 'Outfit_700Bold', color: colors.textPrimary }}>
                  {item.address?.barangay || 'Not Assigned'}
                </Text>
              </Text>
            </View>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <MaterialCommunityIcons name="account-group" size={14} color="#16a34a" />
              <Text style={{ fontSize: 13, fontFamily: 'Outfit_500Medium', color: colors.textSecondary }}>
                Assigned Farmers:{' '}
                <Text style={{ fontFamily: 'Outfit_700Bold', color: colors.textPrimary }}>{assignedFarmers}</Text>
              </Text>
            </View>
          </View>
        )}
