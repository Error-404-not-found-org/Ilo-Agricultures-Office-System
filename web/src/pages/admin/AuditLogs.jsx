...
const DetailValue = ({ value }) => {
  if (value && typeof value === "object") {
    return (
      <pre className="mt-2 max-h-56 overflow-auto whitespace-pre-wrap break-words rounded-box bg-base-100 p-3 text-xs text-base-content/80">
        {JSON.stringify(value, null, 2)}
      </pre>
    );
  }
  return <p className="mt-1 text-sm text-base-content/80">{String(value)}</p>;
};
...
const AuditDetails = ({ log }) => {
  ...
  const entityId = log?.entityId ? String(log.entityId) : null;

  if (!sections.length && !entityId) return null;
    ...
    <div className="mt-3">
      ...
      {expanded && (
        ...
        >
          {entityId && (
            <div className="mb-4">
              <p className="text-xs font-semibold text-base-content/65">Entity ID</p>
              <p className="mt-1 break-all font-mono text-xs text-base-content/80">{entityId}</p>
            </div>
          )}
          <div className="space-y-4">
            {sections.map((section) => (
              <section key={section.label} aria-label={`${section.label} audit values`}>
                <h3 className="text-sm font-semibold text-base-content">{section.label}</h3>
                <dl className="mt-2 space-y-2">
                  {Object.entries(section.value).map(([field, value]) => (
                    <div key={field} className="border-t border-base-300 pt-2 first:border-0 first:pt-0">
...
export default function AuditLogs() {
  ...
  const responsePage = Number.isFinite(Number(data?.page)) ? Number(data.page) : page;
  ...
  return (
    <div className={ui.page}>
      ...
      <main className={ui.main}>
        <section className={`${ui.panelPadded} flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between`}>
          <div>
            <p className="text-sm font-semibold text-base-content/70">Audit entries</p>
            <p className="mt-1 text-2xl font-bold text-base-content" aria-label={`${total} total audit entries`}>
              {isLoading ? "—" : total.toLocaleString()}
            </p>
          </div>
          <div className="flex w-full flex-col gap-3 sm:flex-row lg:w-auto">
            <label className="flex w-full flex-col sm:w-52">
              <span className="mb-1 text-xs font-semibold text-base-content/70">Category</span>
              <select
                value={entityType}
            ...
            <label className="flex w-full flex-col sm:w-72">
              <span className="mb-1 text-xs font-semibold text-base-content/70">Activity</span>
              <span className="input input-sm flex items-center gap-2 bg-base-100 focus-within:outline-2 focus-within:outline-primary">
                <Search size={15} className="shrink-0 text-base-content/55" aria-hidden="true" />
                <input
        ...
        {error && (
          <div role="alert" className="alert alert-error alert-soft sm:alert-horizontal">
            ...
            <button type="button" onClick={() => refetch()} className="btn btn-sm">Retry</button>
          </div>
        )}
        ...
        <section className={ui.panel} aria-label="Audit entries">
          {isLoading ? (
            ...
            <div className="p-10 text-center">
              <FileClock size={28} className="mx-auto text-base-content/45" aria-hidden="true" />
            ...
            <ul className="list divide-y divide-base-300">
              {logs.map((log) => {
                ...
                return (
                  <li key={log._id} className="list-row items-start gap-3 px-5 py-4">
                    ...
                    <article className="min-w-0 list-col-grow" aria-label={formatAuditAction(log.action)}>
                      ...
                      <p className="mt-1 text-xs text-base-content/70">
                        ...
                        <time dateTime={log.createdAt || undefined}>{formatAuditDate(log.createdAt)}</time>
                      </p>
                      {description && <p className="mt-2 text-sm text-base-content/75">{description}</p>}
        ...
        {!isLoading && !error && (
          <nav className="flex flex-col items-center justify-between gap-3 sm:flex-row" aria-label="Audit log pagination">
            ...
            <div className="join">
              ...
              <button
                type="button"
                className="btn btn-sm join-item"
                disabled={page >= responseTotalPages || isFetching}
