...
export default function InseminationTab({
  ...
  return (
    <div className="overflow-x-auto">
      <table className="table w-full min-w-[760px] border-collapse text-xs" aria-label="Insemination ledger records">
        <thead>
            ...
            {columns.map((col) => (
              ...
              >
                <button
                  ...
                  className="flex w-full items-center gap-1 p-3.5 pl-5 text-left hover:text-base-content focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-primary"
                >
                  <span>{col.label}</span>
            ...
            return (
              ...
              >
                ...
                <td className="p-3.5">
                  ...
                  <span className="text-[10px] text-base-content/55 block mt-0.5 max-w-[140px] truncate">
                    {r.detail}
                  </span>
                </td>
                ...
                >
                  <div>
                    <button
                      type="button"
                      popoverTarget={`insemination-ledger-actions-${r.id}`}
                      style={{ anchorName: `--insemination-ledger-actions-${r.id}` }}
                      className="btn btn-ghost btn-circle btn-xs hover:bg-base-200"
                      aria-label={`Actions for record ${r.id}`}
                      aria-haspopup="menu"
                    >
                      <MoreVertical size={16} className="text-base-content/60" />
                    </button>
                    <ul
                      id={`insemination-ledger-actions-${r.id}`}
                      popover="auto"
                      role="menu"
                      aria-label={`Actions for insemination record ${r.id}`}
                      style={{ positionAnchor: `--insemination-ledger-actions-${r.id}` }}
