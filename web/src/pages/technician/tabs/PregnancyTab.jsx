              ...
              >
                ...
                <td className="p-3.5">
                  ...
                  <span className="text-[10px] text-base-content/55 block mt-0.5">
                    Est. Calving: {r.targetCalvingDate || "—"}
                  </span>
                </td>
                ...
                {/* 4. STATUS */}
                <td className="p-3.5">
                  <Badge status={r.result} kind="pregnancy" />
                </td>
                ...
                >
                  <div>
                    <button type="button" popoverTarget={`pregnancy-ledger-actions-${r.id}`} style={{ anchorName: `--pregnancy-ledger-actions-${r.id}` }} className="btn btn-ghost btn-circle btn-xs hover:bg-base-200" aria-label={`Actions for record ${r.id}`} aria-haspopup="menu">
                      <MoreVertical size={16} className="text-base-content/60" />
                    </button>
                    <ul id={`pregnancy-ledger-actions-${r.id}`} popover="auto" role="menu" aria-label={`Actions for pregnancy record ${r.id}`} style={{ positionAnchor: `--pregnancy-ledger-actions-${r.id}` }} className="dropdown dropdown-end menu menu-sm w-44 rounded-box border border-base-300 bg-base-100 p-2 text-base-content shadow-xl">
                      <li role="none">
                        <button
                          type="button"
                          role="menuitem"
