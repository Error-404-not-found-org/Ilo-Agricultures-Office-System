...
import { 
  ...
  CheckCircle2, Clock, MoreVertical, X, AlertCircle, Info, Plus, Calendar as CalendarIcon 
} from "lucide-react";
...
export default function Inseminations() {
  ...
  const fetchAllTasks = async () => {
    try {
      ...
      const formattedInsems = insemData.map((item) => ({
        ...
        date: new Date(item.scheduledDate || item.preferredDate || item.inseminationDate || item.createdAt),
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    else if (filter === "Accepted") passStatus = task.status === "in-progress" || task.status === "approved" || task.status === "approved";
  ...
  const getStatusBadge = (status) => {
    ...
    if (s === "pending") return <span className="px-3 py-1 bg-amber-50 text-amber-600 text-[10px] font-black rounded-full border border-amber-100 uppercase tracking-tighter shadow-sm">Pending</span>;
    if (s === "in-progress" || s === "approved") return <span className="px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black rounded-full border border-emerald-100 uppercase tracking-tighter shadow-sm">In Progress</span>;
    if (s === "done" || s === "resolved") return <span className="px-3 py-1 bg-gray-50 text-gray-500 text-[10px] font-black rounded-full border border-gray-100 uppercase tracking-tighter">Completed</span>;
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-6 mb-10 pt-6">
        <div>
          <div className="flex items-center gap-4 mb-4">
             ...
             <h1 className="text-4xl font-black text-[#111827] tracking-tighter uppercase leading-none">Inseminations</h1>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
             ...
             <table className="table w-full border-separate border-spacing-y-0">
                <thead>
                  <tr className="bg-gray-50/50 text-[#111827] border-b border-gray-100/50">
                    <th className="px-8 py-6 text-left text-[10px] font-black uppercase tracking-[0.2em] opacity-30">Mission Identity</th>
                    ...
                    <th className="px-6 py-6 text-left text-[10px] font-black uppercase tracking-[0.2em] opacity-30">Livestock Info</th>
                  ...
                  {filteredTasks.length === 0 ? (
                    <tr>
                      <td colSpan="6" className="py-32 text-center">
                         ...
                         <h3 className="text-xl font-bold text-gray-300 italic tracking-tight">No AI missions in this sector</h3>
                          ...
                          <td className="px-6 py-7">
                             <div className="flex items-center gap-3">
                                {task.animal?.imageUrl ? (
                                    ...
                                    <div className="w-9 h-9 rounded-xl bg-gray-100 items-center justify-center flex text-gray-400 font-black text-[9px]">TAG</div>
                                )}
                                <div>
        ...
        <div className="xl:col-span-1 space-y-6 flex flex-col h-full self-start sticky top-24">
            ...
            <div className="bg-[#074033] rounded-4xl p-8 shadow-2xl shadow-emerald-900/10 text-white flex flex-col gap-8">
                <div>
                   <h4 className="text-[10px] font-black uppercase tracking-[0.2em] opacity-50 mb-6">Breeding Stats</h4>
                   <div className="space-y-6">
                      <div className="flex justify-between items-end">
                         <div>
                            <p className="text-3xl font-black">{tasks.filter(t => t.status === 'pending').length}</p>
                            <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Pending Requests</p>
                         </div>
                      ...
                      <div className="flex justify-between items-end">
                         <div>
                            ...
                            <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Completed AI Cycles</p>
                         </div>
          ...
          <div className="fixed inset-0 z-60 flex items-center justify-center p-4">
             ...
             >
                ...
                <div className="flex items-center gap-4 mb-10">
                   ...
                   <div>
                      <h3 className="text-2xl font-black text-[#111827] uppercase tracking-tight">Walk-in AI</h3>
                      <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Register immediate AI procedure</p>
                   </div>
                </div>
                ...
                <div className="space-y-6">
                   ...
                   <div>
                      <label className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block ml-1">Sire Code / Breed</label>
                      <input type="text" placeholder="Sire Identity" className="w-full h-14 bg-gray-50 border-none rounded-2xl px-5 font-bold text-sm focus:ring-2 focus:ring-[#074033] transition-all" />
                   </div>
                   <button 
                     onClick={() => { toast.success("Walk-in recorded!"); setIsWalkInModalOpen(false); }}
